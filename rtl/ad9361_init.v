...
module ad9361_init(
	...
	// AD9361 control signal.
	/*这些控制信号用于配置 AD9361 的基本工作模式：
		en_agc = 1'b0：禁用自动增益控制（AGC）
		enable = 1'b0：保持 AD9361 在待机状态（由 SPI 配置激活）
		resetb = ~sys_rst：复位信号（低电平有效）
		txnrx = 1'b0：设置 AD9361 为接收模式
		sync_in = 1'b0：同步输入信号
		ctrl_in = 4'b0000：控制输入为低电平*/
	assign en_agc  = 1'b0;
	assign enable  = 1'b0;
